...
METRIC_GLOSSARY = """\
指標名の日本語表記(必ずこの表記を使うこと。英語の変数名をそのまま出力に書くことは一切禁止):
- total_trades → 総トレード数
- win_rate → 勝率
- profit_factor → プロフィットファクター
- expectancy_pct → 期待値(証拠金対比%・単純平均)
- geometric_expectancy_pct → 期待値(複利ベース)
- compounded_total_return_pct → 複利ベース累計リターン率
- wiped_out → 資金破綻の有無
- average_win_pct → 平均利益率(勝ちトレード)
- average_loss_pct → 平均損失率(負けトレード)
- breakeven_required_gain_pct → 損益分岐に必要な利益率
- breakeven_gap_pct → 実際の平均利益率との差
- breakeven_max_loss_pct → 置くべき損切り上限率(複利ベース)
- breakeven_max_loss_gap_pct → 損切り上限との余裕
- expectancy → 期待値(金額ベース・参考値)
- average_risk_reward_pct → リスクリワード比率(%ベース)
- max_drawdown → 最大ドローダウン
- max_winning_streak / max_losing_streak → 最大連勝数 / 最大連敗数
- average_holding_minutes → 平均保有時間(分)
- rule_adherence_rate → ルール遵守率
- precommit_rate → 事前記録率
- by_currency_pair → 通貨ペア別内訳
- by_entry_reason / by_exit_reason → エントリー理由別 / 決済理由別内訳
...
""" + METRIC_GLOSSARY + """\
厳守事項:
- 単一要素(感情だけ・回数の多寡だけ)で結論を出さないこと。他の要素で説明できないか確認できる範囲で言及すること。
- 「証明する」「因果関係がある」という言い方はしないこと。事実(件数・期待値)の提示に徹し、最終判断はユーザーが行う。
- **出力はすべて日本語で記述すること。英単語(変数名)を一切含めないこと。** 指標に言及する場合は必ず上記の日本語表記を使うこと。「expectancy_pct」「win_rate」のような変数名をそのまま文中に書くことは禁止する。
- **分析はエントリーカテゴリ(by_rule_tag)・決済カテゴリ(by_exit_reason_tag)の中でのみ行うこと。** これらのタグに含まれない要素(時間帯・曜日など)は統計データに含まれていないため、それらを根拠にした提案は一切行わないこと。ユーザーは毎日トレードを行うため、特定の時間帯・曜日を「避ける」という提案は禁止する。もし判断に必要な切り口がタグとして存在しない場合は、結論を述べず「◯◯という条件を切り分けるルールタグの追加を検討」とだけ述べること。
- **すべての提案(entry_improvements等)は、数値の提示だけで終わらせないこと。** 必ず「現状(件数・期待値等の事実)→だから具体的に何をどう変えるか(行動)」の2段構成で1文にまとめること。行動が書けない項目は無理に埋めず省略してよい。
...
{
  "winning_patterns": ["現状(件数・期待値)+ それが示す意味を1文で"],
  "losing_patterns": ["現状(件数・期待値)+ それが示す意味を1文で"],
  "entry_improvements": ["現状 → 具体的な行動を1文で"],
  "stop_loss_improvements": ["現状 → 具体的な行動を1文で"],
  "take_profit_improvements": ["現状 → 具体的な行動を1文で"],
  "avoid_conditions": ["現状 → 具体的な行動を1文で(エントリーカテゴリ内のタグのみ対象)"]
}
"""
...
- リスクリワードや平均利益/損失について言及する場合は、必ず average_win_pct / average_loss_pct / average_risk_reward_pct(すべて%ベース)を使うこと。金額ベースの数値はデータに含まれていないので、円やドルなどの通貨額を使った説明は行わないこと。
- **分析はエントリーカテゴリ(by_rule_tag)・決済カテゴリ(by_exit_reason_tag)の中でのみ行うこと。** これらのタグに含まれない要素(時間帯・曜日など)は統計データに含まれていないため、それらを根拠にした提案・回避提案は一切行わないこと。ユーザーは毎日トレードを行うため、特定の時間帯・曜日を「避ける」という結論は禁止する。もし判断に必要な切り口がタグとして存在しない場合は、結論を述べず「◯◯という条件を切り分けるルールタグの追加を検討」とだけ述べること。
- **単一要素だけで結論を出さないこと。** 例えば「ロング回数が少ないからロングを避ける」「感情だけ」といった、1つの切り口だけを見た短絡的な結論は禁止する。
- あるルールタグで期待値が偏っている場合、それが**他の要素(方向の偏り、他のルールタグの構成、件数)で説明できないか、必ず内訳を確認してから言及すること**。
...
- 件数が少ない条件について言及する場合は、その根拠となる件数と期待値(上記の日本語表記を使うこと)を必ず明記すること。件数が少なく偶然の可能性が高い場合はその旨を明確に述べること。
...
- **出力はすべて日本語で記述すること。英単語(変数名)を一切含めないこと。** 指標に言及する場合は必ず上記の日本語表記を使うこと。「expectancy_pct」「win_rate」のような変数名をそのまま文中に書くことは禁止する。
- **すべての項目は、数値の提示だけで終わらせないこと。** 必ず「現状(件数・期待値等の事実)→だから具体的に何をどう変えるか(行動)」の2段構成で記述すること。行動が導けない項目は「データ不足のため判断不可」とすること。
- **ポジション方向(ロング/ショート)自体を分析対象にしないこと。** ロング/ショート別の期待値比較を根拠にした結論(「ショートの方が期待値が高い」等)を一切述べないこと。方向は利益の要因ではない。
- **感情・メンタル状態を理由に「特定の状態の時はトレードを控える/避ける」という提案をしないこと。** ユーザーはどんな状態でもトレードを行う必要があるため、「メンタルが安定していない時はエントリーを避ける」といった行動制限の提案は禁止する。感情は記録・モニタリングの対象に留め、行動可否のルールにはしないこと。
- **「未入力」(日記が記録されていないだけの状態)を分析対象にしないこと。** 「利確理由が未入力のトレードで損失が多い」のような、データが無いこと自体を根拠にした結論は述べないこと。未入力は単なる記録漏れであり、トレードの条件ではない。
...
{
  "winning_conditions": "期待値が高い条件(該当件数・期待値・関連する内訳)→ それをどう活かすか",
  "losing_conditions": "期待値が低い条件(該当件数・期待値・関連する内訳)→ どう対処すべきか",
  "common_winning_patterns": "複数のルールタグを組み合わせた時に期待値が改善するパターン(単一要素は除く)→ 活用方法",
  "common_losing_patterns": "複数のルールタグを組み合わせた時に期待値が悪化するパターン(単一要素は除く)→ 対処方法",
  "confounding_check": "単一要素で偏りが見えた項目について、内訳を確認した結果(交絡の可能性がある場合はその内容)",
  "rules_to_remove": "削除を検討できるルール(根拠となる期待値・件数)→ 具体的にどう変えるか",
  "rules_to_add": "追加を検討できるルール、またはタグ自体の追加が必要な切り口(根拠となる期待値・件数)",
  "high_expectancy_low_winrate": "勝率より期待値が高い条件 → 活用方法",
  "high_winrate_low_expectancy": "勝率は高いが期待値が低い条件 → 対処方法",
  "top_improvement_priority": "最も期待値改善効果が高いと考えられる課題と、具体的な次の一手",
