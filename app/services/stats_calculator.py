...
def calculate_statistics(trades: List[Trade], leverage_map: Optional[Dict[str, float]] = None) -> dict:
    ...
    return {
        ...
        "average_risk_reward_pct": (
            round(average_win_pct / average_loss_pct, 2)
            if average_win_pct is not None and average_loss_pct not in (None, 0)
            else None
        ),
