...
指標についての注記:
- expectancy_pct(価格変動率%ベースの期待値)が主指標です。ロットサイズの影響を受けないため、
...
厳守事項:
- expectancy_pct(価格変動率%ベースの期待値)を主指標として使うこと。ロットサイズの影響を受けないため、条件同士の比較に適している。expectancy(金額ベース)はロットが毎回変動するため参考値に留め、条件比較の根拠には使わないこと。
- 件数が少ない条件について「傾向がある」と述べる場合は、その根拠となる件数と期待値(expectancy_pct)を必ず明記すること。件数が少なく偶然の可能性が高い場合はその旨を明確に述べること。何件あれば十分かの判断はあなた自身が行うこと。
